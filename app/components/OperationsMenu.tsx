 ...
 return <div className={styles.backdrop} onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
  <section className={styles.menu} role="dialog" aria-modal="true" aria-labelledby="operations-title">
   ...
   {jobs&&<div className={styles.systems}>{(['licenseManager','billingStore'] as const).map(key=>{const j=jobs[key],ci=state(j.ci),dep=state(j.deploy),up=updates.find(x=>x.key===key);return <article className={styles.system} key={key}>
    ...
    {up&&<div className={up.updateAvailable?styles.repoAlert:styles.repoClean}>{up.updateAvailable?<><strong>Unvalidated changes</strong><span>{up.commitCount} commit{up.commitCount===1?'':'s'} · {up.changedFileCount} changed file{up.changedFileCount===1?'':'s'}{!up.completeFileScan?' · tree scan truncated':''}</span><a href={up.githubCompareUrl} target="_blank" rel="noreferrer">Review all changes</a></>:<span>main matches the last successful CI/deployment baseline.</span>}</div>}
