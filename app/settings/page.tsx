...
export default async function Settings({searchParams}:{searchParams?:Promise<{created?:string}>}){
 ...
 return <div className="shell"><SideNav active="settings"/><main className="main">
  ...
  <div className="grid dashboard-metrics api-metrics">
   <div className="card metric-card"><div className="metric-icon icon-green">⚡</div><div><span className="metric-label">Authority services</span><strong className="metric">{liveCount}/{authorityServices.length}</strong><small>{Boolean(s.system_enabled)?'Master authority online':'Master authority offline'}</small></div></div>
