...
export default async function Settings({searchParams}:{searchParams?:Promise<{created?:string}>}){
  ...
  return <div className="shell"><SideNav active="settings"/><main className="main"><div className="top"><div><h1 className="title">System Settings</h1><div className="muted">Local controls owned entirely by this License Manager.</div></div><span className="badge ok">{user.role}</span></div><div className="section"><div className="card"><div className="form">{rows.map(r=><div key={r.field} className="notice"><div style={{display:'flex',justifyContent:'space-between',gap:20,alignItems:'center'}}><div><strong>{r.label}</strong><div className="muted">{r.help}</div></div>{['owner','admin'].includes(user.role)&&<form action={updateSettings}><input type="hidden" name="field" value={r.field}/><button className={`button ${Boolean(s[r.field])?'danger':'secondary'}`} type="submit">{Boolean(s[r.field])?'Turn off':'Turn on'}</button></form>}</div></div>)}</div></div></div>
