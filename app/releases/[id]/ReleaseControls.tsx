...
export default function ReleaseControls({id,reviewStatus,validationStatus,published,channels}:{id:string;reviewStatus:string;validationStatus:string;published:boolean;channels:string[]}){
 ...
 return <div className="release-controls">
  ...
  {!published&&<form action={action}><input type="hidden" name="id" value={id}/><input type="hidden" name="action" value="archive"/><button className="button secondary" disabled={pending}>Archive</button></form>}{channels.length>0&&<form action={action} className="promote-form"><input type="hidden" name="id" value={id}/><input type="hidden" name="action" value="promote"/><select className="input compact-input" name="target_channel" defaultValue=""><option value="" disabled>Promote to…</option>{channels.map(c=><option value={c} key={c}>{c}</option>)}</select><button className="button secondary" disabled={pending}>Promote</button></form>}
