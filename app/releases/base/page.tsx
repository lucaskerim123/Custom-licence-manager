    ...
    <main className="main">
      ...
      <header className="card" style={{ marginBottom: 14, padding: 22 }}>
        <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start', gap: 18 }}>
          <div>
            ...
            <h1 className="title" style={{ marginBottom: 7 }}>Base Deployment</h1>
            <p className="muted" style={{ maxWidth: 760, margin: 0 }}>
              Complete OrbitFS Base candidates arrive from <strong>V1-vercel-base</strong>. License Manager owns technical validation and approval. Final customer publication belongs to Billing Store.
            </p>
          </div>
      ...
      <section className="card" style={{ marginBottom: 14, padding: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginTop: 12 }}>
          {['Build', 'Intake', 'Validate', 'Technical approve', 'Billing publish'].map((step, i) =>
            <div key={step} className="release-pipeline-step">
              <small className="muted">0{i + 1}</small>
              <strong style={{ display: 'block', marginTop: 4 }}>{step}</strong>
            </div>
          )}
        </div>
      </section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 290px', gap: 14, alignItems: 'start' }}>
        <section className="card" style={{ padding: 18 }}>
          <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h2 style={{ margin: 0 }}>Base release queue</h2>
              <p className="muted" style={{ margin: '5px 0 0' }}>No manual candidate creation or customer deployment controls.</p>
            </div>
            <span className="badge">{releases.length} total</span>
          </div>

          {releases.length === 0 ? <p className="muted" style={{ marginTop: 18 }}>No Base release candidates have been captured yet.</p> :
                ...
                return <article key={r.id} className="release-item">
                  <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                    <div>
                      <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <span className="badge">validation {v.status}</span>
                      </div>
                      <p className="muted" style={{ margin: '7px 0 0' }}>
                        {r.source_repo || '—'} · {r.source_ref || '—'} · commit {r.source_sha ? String(r.source_sha).slice(0, 12) : '—'}
                      </p>
                    </div>
                  ...
                  <details style={{ marginTop: 11 }}>
                    ...
                    <div style={{ display: 'grid', gap: 5, marginTop: 9 }}>
                      <small>Release ID: {r.id}</small>
                      <small>Source commit: {r.source_sha || '—'}</small>
                      <small style={{ whiteSpace: 'pre-wrap' }}>Changelog: {r.notes || '—'}</small>
                    </div>
                  </details>
                  {v.checks.length > 0 && <details style={{ marginTop: 7 }}>
                    <summary>Validation checks</summary>
                    <div style={{ display: 'grid', gap: 4, marginTop: 8 }}>
                      {v.checks.map((c: any, i: number) => <small key={c.key || i}>{c.ok ? 'PASS' : 'FAIL'} · {c.message}</small>)}
                    </div>
                  </details>}
                </article>;
              })}
        ...
        <aside style={{ display: 'grid', gap: 11 }}>
          <section className="card" style={{ padding: 16 }}>
            ...
            <p className="muted">License Manager</p>
            <ul className="muted" style={{ paddingLeft: 18, lineHeight: 1.7 }}>
              <li>Intake</li><li>Validation</li><li>Technical approval</li><li>Audit history</li>
            </ul>
          </section>
          <section className="card" style={{ padding: 16 }}>
            <h3 style={{ marginTop: 0 }}>Handoff</h3>
            <p className="muted" style={{ marginBottom: 0 }}>Once technically approved, the candidate is ready for Billing Store's final customer-facing review and publication.</p>
          </section>
        </aside>
      </div>
    </main>
