...
export default async function BaseDeployment(){
  ...
  const [latest,recent]=await Promise.all([
    ...
    db().query(`select r.id,r.version,r.channel,r.status,r.review_status,r.deployment_status,r.source_repo,r.source_ref,r.source_sha,r.artifact_name,r.created_at,r.published_at from releases r join products p on p.id=r.product_id where r.release_type='base' and p.slug='orbitfs_base' order by r.created_at desc limit 20`)
  ]);
