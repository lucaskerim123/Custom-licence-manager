...
export async function GET(request:Request){
  ...
  const rows=(await db().query(`select r.*,p.slug product,p.name product_name from releases r join products p on p.id=r.product_id where p.slug=$1 and p.status='active' and r.channel=$2 and r.review_status='approved' and r.status='published' and r.archived_at is null order by r.created_at desc`,[product,channel])).rows;
