...
export async function GET(request: Request) {
  try {
    ...
    const result = await db().query(`select r.*,p.slug product,p.name product_name from releases r join products p on p.id=r.product_id where p.slug=$1 and ($2='all' or r.channel=$2) and r.release_type=$3 and ($4=true or r.archived_at is null) order by r.published_at desc nulls last,r.created_at desc limit 100`, [product, channel, type, includeArchived]);
