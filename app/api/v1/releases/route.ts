...
export async function POST(request: Request) {
  try {
    ...
    if (!product || !version || !artifactUrl) return NextResponse.json({ error: 'version and artifact_url are required' }, { status: 400 });
    if (releaseType !== 'base') return NextResponse.json({ error: 'BASE_ONLY: this intake currently accepts release_type=base only' }, { status: 400 });
    ...
    if (!settings?.system_enabled || !settings.release_system_enabled || !settings.deployment_enabled) {
      return NextResponse.json({ error: 'AUTHORITY_UNAVAILABLE' }, { status: 503 });
    }
    ...
    const existing = (await pool.query(`select id,status,review_status from releases where product_id=$1 and channel=$2 and version=$3 and release_type='base'`, [p.id, channel, version])).rows[0];
    ...
    const row = await createRelease({
      ...
      sourceRef: body.source_ref ? String(body.source_ref) : 'base-release',
      artifactUrl,
      ...
      artifactRepo: body.artifact_repo ? String(body.artifact_repo) : 'lucaskerim123/V1-vercel-base',
      artifactRunId: body.artifact_run_id ? Number(body.artifact_run_id) : null,
      vercelReady: Boolean(body.vercel_ready),
      supabaseReady: Boolean(body.supabase_ready),
