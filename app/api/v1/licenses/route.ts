...
export async function POST(request:Request){
  try{
    ...
    let expiresAt:Date|null=null;if(body?.expires_at){expiresAt=new Date(String(body.expires_at));if(Number.isNaN(expiresAt.getTime()))return NextResponse.json({error:'INVALID_EXPIRY',code:'INVALID_EXPIRY'},{status:400});}
    ...
    return NextResponse.json({id:result.id,license_key:result.key,status:result.status,expires_at:result.expires_at??null,already_issued:Boolean((result as any).alreadyIssued)});
  }catch(error){console.error('license issue failed',error);const message=error instanceof Error?error.message:'LICENSE_ISSUE_FAILED';return NextResponse.json({error:message,code:'LICENSE_ISSUE_FAILED'},{status:500});}
}
