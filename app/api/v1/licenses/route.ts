...
export async function POST(request:Request){
  ...
  let expiresAt:Date|null=null;if(body?.expires_at){expiresAt=new Date(String(body.expires_at));if(Number.isNaN(expiresAt.getTime()))return NextResponse.json({error:'INVALID_EXPIRY'},{status:400});}
