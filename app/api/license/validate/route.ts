...
export async function POST(request: Request) {
  ...
  const result=await validateLicense({key,productSlug:product,installationId:body?.installation_id?String(body.installation_id):undefined,productVersion:body?.product_version?String(body.product_version):undefined,metadata:body?.metadata&&typeof body.metadata==='object'?body.metadata:undefined});
