...
export default function RunPage(){
 ...
 return <div className={styles.content}>
  ...
  <div className={styles.systems}>
   {systems.map(s=>{const d=data[s.key],run=d?.run,workflow=d?.monitoring||'Workflow',passed=run?.status==='completed'&&run?.conclusion==='success',isCollapsed=!!collapsed[s.key],isConsoleOpen=consoleOpen[s.key]!==false;
    return <article className={styles.system} key={s.key}>
     ...
     {!isCollapsed&&<div className={styles.systemBody}>
      ...
      {run&&<section className={styles.consoleCard}>
       <button className={styles.consoleHeader} onClick={()=>setConsoleOpen(p=>({...p,[s.key]:!isConsoleOpen}))} aria-expanded={isConsoleOpen}><span><strong>LIVE CONSOLE</strong><small>{run.status==='completed'?'Final output':'Live workflow status · jobs and console output refresh every '+(live?'3':'15')+' seconds'}</small></span><span className={styles.consoleIndicator}>{run.status==='completed'?'● CLOSED':'● LIVE'} {isConsoleOpen?'▾':'▸'}</span></button>
       {isConsoleOpen&&<div className={styles.consoleBody}>
        <div className={styles.meta}>{[['Workflow',workflow],['Run','#'+run.run_number],['Commit',run.head_sha],['Updated',time(run.updated_at)],['State',status(run)]].map(([a,b])=><div key={a}><span>{a}</span><strong>{b}</strong></div>)}</div>
