...
async function scanPackage(row:any,bytes:Buffer){
  ...
  try{
    ...
    if(isBundle){
      ...
      const schemaSafe=pkg?.releaseAnalysis?.flags?.schemaChanged!==true;
      ...
      checks.push({key:'package_update_schema',ok:schemaSafe,message:schemaSafe?'Update release does not require a customer database schema migration.':'Update release contains database/schema changes but Update Bundles do not have a migration execution contract. Publish this change through a Base release instead.'});
...
async function checkArtifact(row: any) {
  ...
  try {
    ...
    checks.push(...await scanPackage(row,bytes));
    return { checks };
  } catch (error) {
