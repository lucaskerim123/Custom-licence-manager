-- Normalize License Manager release-channel authority to open/closed access modes.
alter table if exists public.release_channels
  add column if not exists access_mode text not null default 'closed';

update public.release_channels set access_mode='open' where channel='stable';
